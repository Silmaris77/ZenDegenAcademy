        ...
        if st.session_state.lesson_step == 'intro':
            ...
            if 'sections' not in lesson:
                ...
                for section in lesson["sections"]["application"]["sections"]:
                    ...
                    # Dodaj pole do przesyłania rozwiązania
                    title = section.get('title', 'zadanie_praktyczne')
                    task_key = f"application_{title.replace(' ', '_').lower() if title else 'task'}"
                    ...
                    # Dodaj przycisk do zapisywania rozwiązania
                    if st.button("Zapisz rozwiązanie", key=f"save_{task_key}"):
            ...
            if 'sections' not in lesson:
                ...
                if quiz_completed:
                    ...
                    st.markdown("<div class='next-button'>", unsafe_allow_html=True)
                    if zen_button(f"Dalej: {step_names.get(next_step, next_step.capitalize())} (+{earned_points} XP)", use_container_width=False):
            ...
            # Wyświetl podsumowanie lekcji
            if 'summary' not in lesson and 'outro' not in lesson:
                st.error("Lekcja nie zawiera podsumowania!")
            else:
                # Handle both 'summary' and 'outro' fields for compatibility
                summary_content = None
                if 'summary' in lesson:
                    summary_content = lesson['summary']
                elif 'outro' in lesson and isinstance(lesson['outro'], dict) and 'main' in lesson['outro']:
                    summary_content = lesson['outro']['main']
                elif 'outro' in lesson and isinstance(lesson['outro'], str):
                    summary_content = lesson['outro']
                
                if summary_content:
                    st.markdown(summary_content, unsafe_allow_html=True)
                    
                    # Check for case study in outro
                    if 'outro' in lesson and isinstance(lesson['outro'], dict) and 'case_study' in lesson['outro']:
                        st.markdown("### Studium przypadku")
                        st.markdown(lesson['outro']['case_study'], unsafe_allow_html=True)
                else:
                    st.warning("Brak treści podsumowania.")
...
def display_quiz(quiz_data):
    """Wyświetla quiz z pytaniami i opcjami odpowiedzi. 
    Zwraca tuple: (is_completed, is_passed, earned_points)"""
    ...
    if is_completed:
        ...
        # Użyj metody get() dla słownika session_state
        if st.session_state.get('show_xp_notification'):
            st.success(st.session_state.show_xp_notification)
            # Usuń powiadomienie
            st.session_state.show_xp_notification = None
